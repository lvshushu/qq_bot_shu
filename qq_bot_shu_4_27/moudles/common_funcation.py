...
def add_list(file,list_name,target_qq,sessionKey,g_o_f):
    ...
    if int(target_qq) not in group_list:
        ...
        if g_o_f == "guess":
            try:
                message_chain = [{"type": "Plain", "text": ' ' + "游戏开始啦!快来猜数字吧!"}]
                sendmeassage_group(target_qq, message_chain, sessionKey)
            except:
                pass
    else:
    ...
    if g_o_f=="g":
        ...
        message_chain_1 = [{"type": "Plain", "text": f'{list_name} 权限添加成功'}, ]
...
def get_bendi_img_url(img_file,sessionKey):
    ...
    url_2=response['url']
    return url_2
def shut_up(group_qq,qq,sessionKey,t):
    qq = int(qq)
...
def get_image_format(binary_data):#判断图片格式
    try:
...
def message_chain_make(text):
    message_chain=[
        {"type": "Plain", "text": text}
    ]
    return message_chain
def message_chain_make_at(text,At):
    message_chain=[
        {"type": "At", "text": At},
        {"type": "Plain", "text": text}
    ]
